...
function sortByLowestPrice(items: ProductGroup[], sortLow: boolean) {
  if (!sortLow) return items;
  return [...items].sort((a, b) => getPriceRange(a).min - getPriceRange(b).min);
}
...
export function CatalogFlowRecommendations() {
  ...
  const visibleExactGroups = useMemo(() => {
    ...
    return sortByLowestPrice(filtered, sortLow);
  }, [exactGroups, mallOnly, freeShipOnly, sortLow]);
  ...
  const recommendationGroups = useMemo(() => {
    ...
    const sorted = sortByLowestPrice(candidates, sortLow);
    return sorted.slice(0, TARGET_CARD_COUNT - visibleExactGroups.length);
  }, [exactGroups, visibleExactGroups.length, mallOnly, freeShipOnly, sortLow]);
